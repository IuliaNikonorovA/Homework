...
const BsCard = ({
    ...
    return <Card className="pt-3 h-100" id={"pro_" + _id}>
       ...
       {stock>0 ? <Button disabled={inBasket} onClick={addToBasket} variant="warning" className="w-100 position-relative" style={{zIndex: "1"}}>Купить</Button>
      : <Button>Нет в наличии</Button>}
       </Card.Body>
        {user && <Link to={`/product/${_id}`} className="card-link"></Link>}
        {/* {user && <Button as={Link} to={`/product/${_id}`} className="card-link"></Button>} */}
